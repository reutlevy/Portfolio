...
export type NonTupleTExp = AtomicTExp | ProcTExp | TVar |CompoundTExp;
...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    return (pos === -1)  ? parseTuple2(texps) :
...
export const parseTuple2 = (exps: Sexp[]): Result<TupleTExp> => {
const isEmptyTuple = (texps: Sexp[]): boolean =>
    (texps.length === 1) && (texps[0] === 'Empty');
    // [x1 * x2 * ... * xn] => [x1,...,xn]
const splitEvenOdds = (texps: Sexp[]): Result<Sexp[]> =>
...
if(isEmptyTuple(exps)) 
    return makeOk(makeEmptyTupleTExp());
else{
    return bind(splitEvenOdds(exps), (tuple: Sexp[]) => makeOk(makeNonEmptyTupleTExp(map(parseTupleNonEmpty, tuple))));
    }
}

export const parseTupleNonEmpty= (exp: Sexp): NonTupleTExp =>{
    if(exp === "number") return makeNumTExp();
    else if(exp === "boolean") return makeBoolTExp();
    else if(exp === "void")  return makeVoidTExp();
    else if(exp === "string") return makeStrTExp();
    else if(isString(exp)) return makeTVar(exp);
    else if(isProcTExp(exp)) {
        let result=bind(parseTExp(exp), (tuple: TExp) => makeOk(tuple));
        if(isOk(result)){
            return result.value
        }
    }
     else if(isArray(exp)){
         let x=parseCompoundTExp(exp);
         if(isOk(x)) 
             return x.value;
     } 
    return makeTVar("Unexpected TExp ");

...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, CExp, LetValue, Bindings_Values, VarDecl, Values, isValuesExp, isLetValuesExp } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isTupleTExp, TupleTExp, isEmptyTupleTExp, isNumTExp, isTExp, makeNonEmptyTupleTExp, isNonEmptyTupleTExp } from "./TExp";
...
import { Tuple, SExpValue, isTuple } from './L5-value';
...
export const typeofExp = (exp: Parsed, tenv: TEnv): Result<TExp> =>
    ...
    isValuesExp(exp)? typeofvalues(exp.values,tenv) :
...
export const typeofvalues = (exps:CExp[], tenv:TEnv): Result<TExp> =>
     bind(mapResult((x)=> typeofExp(x,tenv) ,exps), (vals: TExp[]) => makeOk(makeNonEmptyTupleTExp(vals)));
...
export const typeofLetValues = (exp:LetValue, tenv:TEnv) : Result<TExp> =>{
    const exps = map((b: Bindings_Values) => b.bindings ,exp.bindings);
    var merged = exps.reduce(function(prev, next) {
        return prev.concat(next);
    });
    const vars = map((b) => b.var, merged);
    const vals = map((b) => b.exp, exp.bindings);
    let vals2:CExp[]=vals.reduce(function (acc: CExp[], curr:CExp) {
        if (isValuesExp(curr)) { return acc.concat(curr.values) }
        else {return acc.concat(curr)}
        return acc;
    }, []);
    const varTEs = map((b) => b.texp, merged);
    const constraints = zipWithResult((varTE, val) => bind(typeofExp(val, tenv),
    (typeOfVal: TExp) => checkEqualType(varTE, typeOfVal, exp)),
     varTEs, vals2);
    return bind(constraints, _ => typeofExps(exp.exp, makeExtendTEnv(vars, varTEs, tenv)));
}

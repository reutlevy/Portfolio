    ...
    import { join, map, zipWith, not, equals } from "ramda";
    import parse, { Sexp, Token } from 's-expression';
    import { isCompoundSExp, isEmptySExp, isSymbolSExp, makeCompoundSExp, makeEmptySExp, makeSymbolSExp, SExpValue, valueToString, isTuple, makeTuple } from './L5-value';
    ...
    */
    ...
    export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | LetValue | Values | LetValue | Bindings_Values | Values
    ...
    export interface LetValue {tag: "LetValue"; bindings: Bindings_Values[] ; exp:CExp[] }
    export const makeLetValue = (bindings: Bindings_Values[], exp:CExp[]): LetValue =>
        ({tag: "LetValue", bindings:bindings , exp:exp});
    export const isLetValuesExp = (x: any): x is LetValue => x.tag === "LetValue";

    export interface Bindings_Values {tag: "Bindings_Values"; bindings: VarDecl[] ; exp:CExp }
    export const Bindings_Values = (bindings: VarDecl[], exp:CExp): Bindings_Values =>
        ({tag: "Bindings_Values", bindings:bindings , exp:exp});
    export const isBindings_Values = (x: any): x is Bindings_Values => x.tag === "Bindings_Values";

    export interface Values {tag: "Values"; values: CExp[]}
    export const makeValues = (values: CExp[]): Values =>
        ({tag: "Values", values:values});
    ...
    export const parseL5SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
        isEmpty(params) && ((equals(op,"let-values"))) ? makeFailure("Empty args for special form") :
    ...
    export const parseletvalues =(first: Sexp, rest:Sexp[]): Result<LetValue> =>
        isEmpty(rest) ? makeFailure('Body of "letvalue" cannot be empty') :
       ! isGoodBindings(first) ? makeFailure('The bindings are not good') :
       safe2((bdgs: Bindings_Values[], body: CExp[]) => makeOk(makeLetValue(bdgs, body)))
           ((parseBinding_Values(first)), mapResult(parseL5CExp, rest));


     const parseBinding_Values = (bindings: Sexp[]): Result<Bindings_Values[]> =>
       mapResult((val) => isArray(val[0]) ? 
             parseBindValue(val[0], val[1]) :
           makeFailure("not good bindings"), bindings) 
    
    const parseBindValue = (val:Sexp[], var1:Sexp) : Result<Bindings_Values> =>
        safe2((bdgs: VarDecl[], body: CExp) => makeOk(Bindings_Values(bdgs, body)))
         (mapResult(parseVarDecl, val), parseL5CExp(var1))
    ...
    */
    ...
    const isSpecialForm = (x: string): boolean =>
        ["if", "lambda", "let", "quote", "letrec", "set!", "let-values", "values"].includes(x);
    ...
    export const parseSExp = (sexp: Sexp): Result<SExpValue> =>
        ...
        isArray(sexp) && sexp.includes("values") ? bind(mapResult(parseSExp, sexp), (exps: SExpValue[]) => makeOk(makeTuple(exps))):
        isArray(sexp) ? (
            // fail on (x . y z)
    ...
    export const unparse = (e: Parsed): Result<string> =>
        ...
        isLetValuesExp(e) ? unparseLetValue(e) :
        isBindings_Values(e) ? unparse_Bindings_Values([e]) :
        isValuesExp(e) ? unparseValues(e) :
        isProgram(e) ? bind(mapResult(unparse, e.exps), (exps: string[]) => makeOk(`(L5 ${exps})`)):
        makeFailure("Not an expression")
    ...
    const unparse_Bindings_Values = (bindings: Bindings_Values[]): Result<string> =>
        bind(mapResult(bdg => safe2((vd: string[], val: string) => makeOk(`((${join(" ", vd)}) ${val})`))(mapResult(unparseVarDecl, bdg.bindings), unparse(bdg.exp)), bindings),
                (bdgs: string[]) => makeOk(join(" ", bdgs)));
    ...
    const unparseLitExp = (le: LitExp): string =>
        ...
        isTuple(le.val) ? `'${valueToString(le.val)}` : 
        `${le.val}`;
    ...
    const unparseValues = (pe: Values): Result<string> =>
        bind(mapResult(unparse, pe.values), (les: string[]) => makeOk(`(values ${join(" ", les)})`));
    ...
    const unparseLetValue = (le: LetValue): Result<string> =>
        safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
            (unparse_Bindings_Values(le.bindings), unparseLExps(le.exp));

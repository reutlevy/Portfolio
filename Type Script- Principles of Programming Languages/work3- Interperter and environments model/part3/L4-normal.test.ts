...
describe('L4 Normal Eval', () => {
    ...
    // Test that we added - part 3.3
    
    it('test 1-Normal eval will not have any side-effects while applicative eval will display "diffrenet" to the screen. They both will return the value 10', () => {
        expect(bind(parseL4(
       `(L4 (define diffrent (lambda ()
        (display 'diffrent)
        0 ))
       (define test (lambda (x) 10))
       (test (diffrent)))`), evalNormalProgram)).to.deep.equal(makeOk(10));  
    });

    it('test 2- normal-eval will return 12 while applicative-eval will get into infinite loop', () => {
        expect(bind(parseL4(
       `(L4 (define loop (lambda (x) (loop x)))
       (define looper (lambda (x y) y))
       (looper (loop 0) 12))`), evalNormalProgram)).to.deep.equal(makeOk(12));  
    });

    it('test 3- normal-eval will return 8 while applicative-eval will return divide by zero error', () => {
        expect(bind(parseL4(
       `(L4
       (define test
         (lambda (x y)
       ...
       (test 0 (zero-divide 5)))`), evalNormalProgram)).to.deep.equal(makeOk(8));  
    });

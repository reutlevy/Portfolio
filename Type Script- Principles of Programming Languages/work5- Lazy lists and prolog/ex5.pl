...
%
pick_me_up(Child_name,Phone) :- parents(Child_name,X,Y), parent_details(X,Phone,true) | parent_details(Y,Phone,true)
...
% Purpose: a relationship between an activity name and list of all the childrens names that participate at this activity (without repetition)
%
activity_participants_list(Name, List) :- bagof(X,participate(X,Name), List).
...
%
can_register(child_name,Activity) :- activity(Activity,day), not_member(day,List), findall(X,(participate(child_name,A),activity(A,X)),List).
